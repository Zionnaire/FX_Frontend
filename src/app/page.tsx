...
// ─── MiniChart with live candle updating ─────────────────────────────────────
function MiniChart({ data, pair, livePrice }: { data: any[]; pair: string; livePrice?: number | null }) {
  ...
  // Live candle update — mutates last bar's high/low/close without re-creating the chart
  useEffect(() => {
    if (!seriesRef.current || livePrice == null || !lastBarRef.current) return;
...
// ─── Live price ticker with flash animation ───────────────────────────────────
function LiveTicker({ pair }: { pair: string }) {
  const { price, direction } = useRealtimePrice(pair);
  const [flashKey, setFlashKey] = useState(0);

  useEffect(() => {
    if (direction != null) setFlashKey((k) => k + 1);
  }, [price, direction]);

  const fmt = getPriceFormat(pair);
  const formatted = price != null ? price.toFixed(fmt.precision) : '—';
  const color = direction === 'up' ? 'var(--acc)' : direction === 'down' ? 'var(--red)' : 'var(--t1)';
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span
        key={flashKey}
        className={`text-sm font-bold font-mono-num px-1 rounded ${flashClass}`}
        style={{ color }}
      >
        {formatted}
      </span>
          ...
          <div className="card p-3" style={{ height: 260 }}>
            ...
            <div style={{ height: 210 }}>
              {chart && chart.length > 0
                ? <MiniChart data={chart} pair={activePair} livePrice={livePrice} />
                  ...
                  : recentTrades.map((t: any) => (
                    <tr key={t._id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td className="py-1.5 text-right font-mono-num"
                        style={{ color: t.pnl > 0 ? 'var(--green)' : t.pnl < 0 ? 'var(--red)' : 'var(--t2)' }}>
